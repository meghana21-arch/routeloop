...
export default function Home(){
  ...
  return <main className="shell">
    <aside className={`sidebar ${open?'open':''}`}>
      ...
      <div className="side-foot"><a href="#settings"><Settings/>Settings</a><div><strong><i/>Demo environment</strong><p>Reproducible seed · read only</p></div></div>
    </aside>
    <section className="main">
      ...
      <div className="content" id="overview">
        ...
        <section className="bottom-grid">
          ...
          <article className="panel traces" id="traces"><div className="panel-head"><div><h2>Recent traces</h2><p>Latest routing decisions</p></div><a href="#traces">View all <ArrowRight/></a></div>{traces.map(t=><div className="trace" key={t[0]}><i className={t[5]}/><b>{t[0]}<small>{t[1]}</small></b><span>{t[2]}<small>{t[3]}</small></span><time>{t[4]}</time></div>)}</article>
        </section>
        <footer><span><Logo/>RouteLoop</span><p>Real routing. Reproducible demo data. No production prompts stored.</p><span><i/>Gateway healthy · 3 providers</span></footer>
      </div>
    </section>
  </main>
}
